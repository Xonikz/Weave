...
Weave is currently undergoing active development to evolve from a functional RAG script into a professional-grade Local ETL (Extract, Transform, Load) data manager. Version 2.0 updates will focus on UI immersion, data accountability, and automated multi-modal ingestion.
...
* **Decoupled Background Worker:** Moving the ingestion engine to an invisible Python thread, allowing the user to close the browser or navigate the UI while Weave processes data in the background.
* **Batch CSV Ingestion:** Allowing users to drop a CSV of file paths into Weave to automate massive data imports.
